...
function Pics() {
	return C('div',{id:'pictop'},[
		C('div',{id:'pics'},['pics'])
	]);
}

function Tables({searches=[],local=[],display}) {
	return C('div',{id:'tables',style:{display}},[
		C('div',{},local.map(s=>C('div',{},[
			C('span',{},[s.name]),C('button',{},['X'])
		]))),
		C('div',{},searches.map(s=>C('div',{},[
			C('span',{},[s.name]),C('button',{},['X'])
		]))),
	]);
}
...
function InterContainer({tables,current,local}) {
	return C('div',{id:'intercontainer'},[
		...
		C('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr'}},[
			C('button',{id:'show-tables',listeners:{onclick:()=>tables()}},[`\u2606: ${local}`]),
			C('button',{id:'savLocBtn'},[`\u2602 save this place`]),
			C('button',{id:'loc'},[`\u2600`])
		])
...
function Frame({options,session}) {
	return C('div',{id:'loadframe'},[!options?Loading():session.pending?iFrame():Loading(getLocation())]);
	function iFrame() {
		return C('iframe',{id:"forecast_embed",type:"text/html",frameborder:0,height:245,width:"100%",src:session.pending});
	}
	function Loading() {
		...
		navigator.geolocation.getCurrentPosition(function(p) {
			...
			geocoder.geocode({'location': {lat: p.coords.latitude, lng: p.coords.longitude}}, function(results, status) {
				if (status === 'OK') {
					...
					var src = "https://forecast.io/embed/#lat=" + p.coords.latitude.toFixed(4) + "&lon=" + p.coords.longitude.toFixed(4) + "&name=" + session.current.name;
					session.pending = `${src}&color=${options.color}&font=${options.font}&units=${options.units}`;
					// geosrc(session.current.name);
					session.geocode = true;
					session.searches.push({name:session.current.name,src,coords:session.current.coords});
					C.sync();
				}
			})
		}, function(error) {
...
var App = (function() {
	getLocal();
	var data = {};
	var state = {tableDisplay:''};
	data.session = JSON.parse(sessionStorage.getItem('DSNT'))||{};
	if (!data.session.searches) {
	...
	return function() {
		return C('div',{},[
			...
			Frame({options:data.local.options,session:data.session}),
			InterContainer({tables,current:data.session.current,local:data.local.saved?data.local.saved.length:''}),
			Tables({display:state.tableDisplay,searches:data.session.searches,local:data.local.saved}),
			Pics(),
			Options()
		]);
	}
	function getLocal() {
		chrome.storage.sync.get(null,(local) => {
			if (typeof local === 'string') {
				local = JSON.parse(local);
			}
			data.local.options = local.options || {backgroundColor: '#9C9C9C',font: 'Helvetica',units: '',color: '#333333'};
			data.local.saved = local.saved || [];
			document.body.style.backgroundColor = data.local.options.backgroundColor;
			document.body.style.fontFamily = data.local.options.font;
			C.sync();
		});
	}
	function tables() {
		state.tableDisplay = state.tableDisplay === 'none' ? '' : 'none';
		C.sync();
	}
}());
